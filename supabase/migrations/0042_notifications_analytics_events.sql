-- Modulo F: amplia o allowlist fechado de analytics_events.event_name (ver
-- 0031_analytics_tips_events.sql) com os eventos de notificacoes. Mesma
-- tecnica ja usada em 0029/0031: drop + recreate do CHECK constraint e do
-- corpo de record_analytics_event() (a assinatura da funcao nao muda,
-- entao aqui e create or replace simples, nao precisa dropar primeiro).
--
-- Eventos disparados pelo CLIENTE (permissao concedida/negada, subscription
-- criada/revogada, notificacao aberta/lida/clicada) continuam passando por
-- este RPC normalmente - a sessao do usuario resolve auth.uid() sozinha.
-- Eventos disparados pelo PROCESSADOR EM LOTE (campanha criada/agendada,
-- envio, falha) sao inseridos diretamente pelo client service-role (ver
-- src/server/services/notification-analytics.service.ts), sem passar por
-- este RPC - o processador roda para varios usuarios por vez e este RPC
-- sempre usa auth.uid() da sessao chamadora, que e nulo sob service role.
-- O CHECK constraint da tabela e a garantia real em ambos os casos.
