-- Real gap found while validating 0037 against a real account (QA fixture
-- enrollment on the real August challenge, never the 3 real participants):
-- challenge_first_habit_completed never fires anymore. That event lived
-- inside update_habit_log() (fired once when completed_habits_lifetime
-- transitioned to exactly 1) - since the new batched flow upserts
-- habit_logs directly inside finalize_daily_log_with_responses and never
-- calls update_habit_log at all, the event was silently dropped. Confirmed
-- by finalizing a real day for the QA enrollment and checking
-- analytics_events: only challenge_day_completed was recorded.
--
-- admin_challenge_funnel's "first_habit" stage (0032) and the
-- "primeiro-habito" achievement's funnel-adjacent analytics both read this
-- event, so this is a real regression, not cosmetic. Fix: fire it from
-- finalize_daily_log_with_responses itself, using the exact same
-- completed_habits_lifetime = 1 condition update_habit_log used - moved,
-- not reinvented. Nothing else in the function changes.
...
begin
  ...
  -- FIX: this event used to fire from update_habit_log() the moment a
  -- member's lifetime completed-habit count first reached 1 - the batched
  -- flow never calls that function anymore, so it has to fire here instead,
  -- using the exact same condition/values.
