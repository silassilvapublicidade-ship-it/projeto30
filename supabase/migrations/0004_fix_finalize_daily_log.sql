-- Fixes finalize_daily_log(), which has never succeeded in production:
-- it referenced the local variable `points_earned` as
-- `finalize_daily_log.points_earned` to disambiguate it from the
-- daily_logs.points_earned column, but the function body has no
-- <<finalize_daily_log>> block label, so Postgres read that qualifier as
-- an unknown table and raised "missing FROM-clause entry for table
-- finalize_daily_log". This mirrors the same class of bug already fixed
-- for journey_recalculate_daily_log in 0003_fix_daily_log_recalculate.sql:
-- the variable is renamed (v_points_earned) instead of qualified.
--
-- Also adds the missing server-side guard: finalize_daily_log() must
-- refuse to finalize a day while a required habit has no `completed`
-- habit log (not_applicable does not count for required habits). This
-- raises a distinct, stable error (SQLSTATE 'P0003') before any
-- point_events, daily_logs or challenge_enrollments write happens, so a
-- blocked finalize call has zero side effects.
