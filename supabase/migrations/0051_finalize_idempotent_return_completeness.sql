-- Rodada "experiencia diaria" - achado colateral durante a auditoria do
-- streak (Parte A/3, "finalizacao idempotente"): o branch already_finalized
-- de finalize_daily_log_with_responses nunca devolvia applicable_habits/
-- completed_habits, embora o tipo TS do cliente (RawFinalizeSummary) os
-- declare obrigatorios - uma finalizacao repetida (o mesmo dia sendo
-- finalizado de novo, ex.: duplo clique/retry de rede) devolvia esses dois
-- campos como undefined em tempo de execucao. journey_recalculate_daily_log
-- e idempotente por natureza (so relê habit_logs e regrava
-- completion_percent com o mesmo valor) - reaproveitada aqui em vez de
-- duplicar a contagem. Nenhuma outra linha da funcao muda.
...
comment on function public.finalize_daily_log_with_responses(uuid, jsonb) is
  'Same as 0050 (visibilidade/streak inalterados) - already_finalized agora '
  'tambem devolve applicable_habits/completed_habits (faltavam antes, '
  'apesar do tipo TS do cliente ja os exigir).';
