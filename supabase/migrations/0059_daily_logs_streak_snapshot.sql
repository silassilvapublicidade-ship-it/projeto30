-- Dashboard de Evolucao Pessoal (Perfil) - Parte 1/3.
--
-- AUDITORIA (antes de adicionar qualquer coisa): a timeline de evolucao
-- pedida no briefing inclui "novo recorde de sequencia" como um tipo de
-- evento real. challenge_enrollments.streak_current/streak_best sao valores
-- MUTAVEIS (sempre o estado atual, nunca um historico) - nao ha como saber,
-- so olhando o estado de hoje, em qual dia especifico um recorde historico
-- foi batido. Reconstruir isso recalculando o algoritmo de streak inteiro
-- por fora (fora de finalize_daily_log_with_responses) seria arriscado:
-- corre o risco de divergir sutilmente da logica autoritativa (respeito a
-- pausas, expected_date, minimo de conclusao) - exatamente o tipo de bug
-- que a rodada anterior (Silas, Dia 2) already caused real confusion.
--
-- Solucao adotada, de baixo risco: uma unica coluna nova, nullable,
-- opcional, que so GRAVA (nunca recalcula) o valor de streak_count que
-- finalize_daily_log_with_responses JA calcula internamente, no exato
-- momento em que o dia e finalizado - a mesma fonte autoritativa, nunca uma
-- segunda implementacao. Dias ja finalizados antes desta migration ficam
-- com streak_at_finalize = null (sem backfill retroativo, para nao arriscar
-- recalcular incorretamente o historico de ninguem) - a timeline so passa a
-- detectar recordes a partir de finalizacoes feitas apos este deploy,
-- documentado explicitamente no relatorio final.

alter table public.daily_logs
  add column if not exists streak_at_finalize integer;

comment on column public.daily_logs.streak_at_finalize is
  'Snapshot do streak_count calculado por finalize_daily_log_with_responses '
  'no momento exato desta finalizacao - nunca recalculado depois. Null para '
  'dias finalizados antes desta coluna existir (sem backfill retroativo, '
  'Dashboard de Evolucao Pessoal). Usado apenas por '
  'member_profile_timeline() para detectar "novo recorde de sequencia" '
  '(streak_at_finalize deste dia > max(streak_at_finalize) de dias '
  'anteriores da mesma inscricao) - nunca lido por nenhuma regra de negocio.';

-- Redefine finalize_daily_log_with_responses: corpo IDENTICO ao de 0051,
-- com uma unica linha adicionada (streak_at_finalize = streak_count) dentro
-- do UPDATE que ja marca o dia como finalizado. Nenhuma outra linha muda -
-- nenhum calculo de pontos, streak, conquista ou validacao e alterado.
...
begin
  ...
  if daily_record.status = 'finalized' then
    ...
    -- Idempotente por natureza (so relê habit_logs e regrava
    -- completion_percent com o mesmo valor) - reaproveitada aqui para
    -- preencher applicable_habits/completed_habits, que este branch nunca
    -- devolvia.
  ...
  -- UNICA LINHA NOVA desta redefinicao: streak_at_finalize = streak_count
  -- (calculado logo abaixo, mesma variavel, mesmo valor que o retorno da
  -- funcao ja usava). Nenhuma outra linha deste UPDATE muda.
  ...
  -- Grava o snapshot agora que streak_count esta calculado - mesma
  -- transacao, mesmo UPDATE que ja seta status/finalized_at (ver comentario
  -- acima; este e o unico ponto novo de fato, movido para depois do calculo
  -- de streak_count porque esse valor so existe a partir daqui).
  update public.daily_logs
  set streak_at_finalize = streak_count
  where id = daily_record.id;
...
comment on function public.finalize_daily_log_with_responses(uuid, jsonb) is
  'Same as 0051 (nenhuma regra de pontos/streak/conquista muda) - grava '
  'daily_logs.streak_at_finalize = streak_count no momento da finalizacao, '
  'para o Dashboard de Evolucao Pessoal (member_profile_timeline) detectar '
  '"novo recorde de sequencia" sem recalcular o historico por fora.';
