-- Real gap found while auditing the achievement-unlock flow end to end
-- (Silas reported achievements not unlocking - turned out to be residue from
-- manual testing on his real account, see round report, but the audit
-- surfaced this separate, genuine gap): finalize_daily_log_with_responses's
-- unlocked_achievements payload only ever included achievement_record.id -
-- the static achievements.id - never the user_achievements.id row that was
-- just inserted for THIS unlock. GET /api/achievements/[userAchievementId]/
-- share-card (achievement-art.service.ts) requires that user_achievements.id
-- to look up the unlocked row and generate/reuse its card image - the
-- achievement.id alone can't be used for that lookup (ownership check would
-- fail). This blocked any client-side "unlock just happened, show the card
-- and let them share it immediately" flow from ever working, since the
-- client never received the one id it needed to call that endpoint.
--
-- Fix: capture the inserted user_achievements.id via `returning id into`
-- and add it to the payload as user_achievement_id. Nothing else changes -
-- same conditions, same points, same analytics, same idempotent early
-- return.
...
comment on function public.finalize_daily_log_with_responses(uuid, jsonb) is
  ...
  'call - same behavior as 0038, but unlocked_achievements entries now also '
  'carry user_achievement_id (the user_achievements.id row just inserted), '
  'required by the share-card endpoint. Idempotent: an already-finalized '
  'day returns early with an empty unlocked_achievements list.';
