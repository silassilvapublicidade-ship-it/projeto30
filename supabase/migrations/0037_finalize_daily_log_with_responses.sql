-- Batches every habit response (and its comment) for a day into ONE
-- transactional RPC, called only when the member taps "Finalizar o dia".
-- Replaces the previous per-click save-on-every-tap architecture
-- (update_habit_log called once per button press) for the Hoje screen -
-- update_habit_log itself is untouched and still callable (nothing else in
-- the schema depends on removing it), but the UI no longer calls it.
--
-- AUDITORIA PREVIA (ver relatorio da rodada): confirmado lendo
-- journey_recalculate_daily_log (0009) e finalize_daily_log (0036) que:
--   - completion_percent do dia e o all_habits_bonus_points JA ignoram
--     habitos weekly/monthly (filtro h.frequency_type = 'daily') - nao
--     precisa mudar essa formula.
--   - o motor ja soma progresso semanal/mensal a partir de habit_logs
--     existentes (getPeriodProgressByHabitId/getRecurringHabitProgress no
--     TypeScript) - nao precisa de nenhuma tabela ou coluna nova aqui.
--   - pontos, streak e desbloqueio de conquistas sao calculados inteiramente
--     dentro de finalize_daily_log a partir do que esta em habit_logs no
--     momento da finalizacao - portanto bastava fazer upsert de TODOS os
--     habit_logs do dia antes de rodar exatamente a mesma logica de
--     finalize_daily_log, sem duplicar nem reescrever formula nenhuma.
--
-- Pedido explicito desta rodada: remover a trava "Conclua todos os habitos
-- essenciais do dia antes de finalizar" (missing_required_habits) por
-- completo - o dia agora pode ser finalizado com pendencias; qualquer
-- habito aplicavel sem resposta do cliente e normalizado para 'pending'
-- (nao gera pontos, nao conta como concluido) em vez de bloquear.
...
begin
  ...
  -- Idempotency: a duplicate click, a retry after a slow/dropped response,
  -- or the browser resending the same request all land here once the first
  -- call already committed - return the same summary again, do nothing
  -- twice. This is the same early-return finalize_daily_log has always had;
  -- it already covers "clique duplo" and "retry" without any extra
  -- bookkeeping table.
  ...
  -- Upsert every habit applicable to this day in one pass. Server truth
  -- (challenge_day_habits/habits) drives the loop, never the client's
  -- payload - an unknown/stale habit_id in `responses` is silently ignored
  -- (it never appears in day_habits), and a habit the client's payload
  -- omitted entirely still gets a row here, normalized to 'pending'. A
  -- malformed habit_id string, an unrecognized status, or a not_applicable
  -- sent for a required habit are all normalized rather than raising - this
  -- RPC never aborts the whole finalize over one bad response.
  for habit_row in
  ...
  loop
    if habit_row.habit_type in ('quantity'::public.habit_type, 'duration'::public.habit_type) then
      ...
      -- text/single_choice/multiple_choice: not supported by this journey
      -- yet (same as update_habit_log), kept a harmless empty value rather
      -- than aborting the whole finalize over an unsupported habit type.
      normalized_value := '{}'::jsonb;
    end if;
  ...
  -- Everything below is byte-for-byte finalize_daily_log's existing body
  -- (0036) - same completion recalculation, same points/bonus/streak
  -- formulas, same achievement conditions, same analytics events. The only
  -- structural difference from 0036 is that the missing_required_habits
  -- gate is gone entirely (removed per this round's explicit request, not
  -- just exempting weekly/monthly like 0036 did) and habit_logs are already
  -- upserted above instead of via a prior separate update_habit_log call.
  ...
  -- Analytics: identical event set to finalize_daily_log, fired only after
  -- everything above committed successfully - no per-tap events exist in
  -- this flow at all (there's nothing to fire on a local-only tap), so a
  -- mark the member undoes before finalizing was never recorded.
...
comment on function public.finalize_daily_log_with_responses(uuid, jsonb) is
  'Batches all habit responses (status + comment) for a day into one '
  'transactional finalize call - upserts habit_logs for every habit '
  'applicable to the day (unanswered ones normalized to pending), then runs '
  'the exact same points/streak/achievement/analytics logic as '
  'finalize_daily_log. No required-habit gate: a day can be finalized with '
  'pending habits, same as finalize_daily_log after this migration no '
  'longer applies. Idempotent: calling it again on an already-finalized day '
  'returns the same summary without re-running any side effect.';
