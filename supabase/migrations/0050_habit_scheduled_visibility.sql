-- Rodada "experiencia diaria" - Parte C: visibilidade programada de
-- habitos/acoes especiais.
--
-- AUDITORIA (confirmada com dados reais da conta do Silas, so leitura):
-- "Concluir o livro do mes", "Realizar duas acoes de autocuidado" e "Fazer
-- musculacao 4 vezes por semana" ja existem como public.habits normais
-- (nao ha tipo "action" nem estrutura paralela) e ja aparecem em TODOS os
-- dias via public.challenge_day_habits (uma linha por dia, geradas em bloco
-- por admin_generate_challenge_days) - frequency_type ('weekly'/'monthly')
-- ja excluia esses itens do denominador da porcentagem diaria
-- (journey_recalculate_daily_log, migration 0009), mas nao controlava
-- QUANDO o item aparece/pode ser respondido - so o calculo de percentual.
-- Nao existe hoje nenhum mecanismo de agendamento por dia. Decisao: nao
-- criar uma tabela nova nem um tipo de item paralelo - challenge_day_habits
-- continua sendo gerado para todos os dias (nenhuma mudanca ai), e um novo
-- campo em habits (visibility_config) decide, para cada dia, se aquela
-- linha esta "aplicavel" - resolvido em uma unica funcao pura, reaproveitada
-- tanto pelo calculo de porcentagem quanto pela finalizacao, nunca so no
-- cliente/CSS.
--
-- Default '{"type":"all_days"}' para toda linha existente preserva
-- exatamente o comportamento atual - nenhum habito publicado muda de
-- comportamento sem reconfiguracao explicita do admin.

alter table public.habits
  add column if not exists visibility_config jsonb not null default '{"type": "all_days"}'::jsonb;

alter table public.habits
  drop constraint if exists habits_visibility_config_check;
alter table public.habits
  add constraint habits_visibility_config_check check (
    visibility_config ? 'type'
    and (
      (visibility_config ->> 'type') in ('all_days', 'first_day', 'last_day')
      or (
        (visibility_config ->> 'type') = 'from_day'
        and jsonb_typeof(visibility_config -> 'day') = 'number'
      )
      or (
        (visibility_config ->> 'type') = 'between_days'
        and jsonb_typeof(visibility_config -> 'from') = 'number'
        and jsonb_typeof(visibility_config -> 'to') = 'number'
      )
      or (
        (visibility_config ->> 'type') = 'specific_days'
        and jsonb_typeof(visibility_config -> 'days') = 'array'
      )
    )
  );

comment on column public.habits.visibility_config is
  'Quando este item aparece/pode ser respondido no ciclo. Tipos: all_days '
  '(default, sempre), first_day, last_day (dia = duration_days do desafio), '
  'from_day ({"day": N}, a partir do dia N inclusive), between_days '
  '({"from": N, "to": M}, inclusive), specific_days ({"days": [N, M, ...]}). '
  'Resolvido por public.habit_visible_on_day(), nunca so no cliente/CSS - '
  'ver journey_recalculate_daily_log e finalize_daily_log_with_responses.';

-- Funcao pura (sem tabela, sem RLS) - mesma classe de journey_calculate_day
-- (0002). day_number/duration_days sempre vem de challenge_days/challenges
-- reais, nunca de input do cliente.
create or replace function public.habit_visible_on_day(
...
as $$
  select case (p_visibility_config ->> 'type')
    when 'all_days' then true
    when 'first_day' then p_day_number = 1
    when 'last_day' then p_day_number = p_duration_days
    when 'from_day' then p_day_number >= (p_visibility_config ->> 'day')::integer
    when 'between_days' then
      p_day_number >= (p_visibility_config ->> 'from')::integer
      and p_day_number <= (p_visibility_config ->> 'to')::integer
    when 'specific_days' then
      exists (
        select 1
        from jsonb_array_elements_text(p_visibility_config -> 'days') as d(value)
        where d.value::integer = p_day_number
      )
    -- tipo desconhecido (nunca deveria acontecer, dado o CHECK constraint em
    -- habits) cai para sempre visivel - nunca some um item por um valor
    -- inesperado.
    else true
  end;
$$;

revoke all on function public.habit_visible_on_day(jsonb, integer, integer) from public, anon;
grant execute on function public.habit_visible_on_day(jsonb, integer, integer) to authenticated, service_role;

-- journey_recalculate_daily_log: mesma logica de 0009 (so 'daily' entra no
-- denominador), agora tambem exigindo que o item esteja visivel neste dia -
-- um item 'daily' fora da janela configurada (ex.: "fotos de evolucao" so
-- no dia 1 e 31, mas cadastrado como frequency_type='daily' porque nao e
-- recorrente) nunca conta no denominador dos outros dias.
...
-- finalize_daily_log_with_responses: identico a 0049, exceto o loop que
-- grava habit_logs - agora sempre resolve visibilidade no servidor e forca
-- 'not_applicable' para qualquer item fora da janela configurada, mesmo que
-- o cliente tenha enviado 'completed' para ele (nunca confia no que o
-- cliente diz sobre um item que nao deveria nem estar respondendo). Um item
-- nao visivel nunca gera pontos (o loop de pontos ja exige status =
-- 'completed', que este item jamais recebe) e nunca bloqueia a
-- finalizacao (fica 'not_applicable', que e sempre um status valido).
...
begin
  ...
  challenge_day_number := daily_record.challenge_day_number;
  ...
  for habit_row in
    ...
    select
      ...
      cdh.required,
      case
        -- Nao visivel hoje: o servidor decide sozinho, o cliente nunca pode
        -- forcar 'completed' para um item fora da janela configurada.
...
comment on function public.finalize_daily_log_with_responses(uuid, jsonb) is
  'Same behavior as 0049 (streak calculo/retorno inalterado), agora tambem '
  'forcando not_applicable no servidor para qualquer habito fora da janela '
  'de habit_visible_on_day() - o cliente nunca pode responder um item nao '
  'aplicavel ao dia, e um item assim nunca entra no denominador nem bloqueia '
  'a finalizacao.';
